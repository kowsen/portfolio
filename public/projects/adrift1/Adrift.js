/**
 * Sets up a PointerEvent interceptor on the canvas that feeds synthesized
 * TouchEvents to a specific handler.
 * * This function uses a "Singleton" pattern on the canvas: the first time it is called,
 * it attaches the actual PointerEvent listeners. Subsequent calls simply register
 * the provided handler into the internal list for that event type.
 * * @param {HTMLCanvasElement} canvas - The game canvas.
 * @param {Function} targetHandler - The function to call when the event fires.
 * @param {string} eventName - The specific event to listen for ('touchstart', 'touchmove', 'touchend', 'touchcancel').
 */
function setupPointerToTouchConversion(canvas, targetHandler, eventName) {
  // 0. Polyfill Touch and TouchEvent for Desktop Firefox/Safari
  // These browsers often hide these constructors entirely on desktop,
  // causing 'ReferenceError: Touch is not defined' when we try to synthesize events.
  ...
  // 1. CSS Requirement: Prevent browser panning/zooming.
  canvas.style.touchAction = "none";

  // 2. Initialization: Check if we have already set up the interceptor on this canvas.
  if (!canvas._pointerToTouchState) {
    // Initialize the shared state object attached to the DOM element
    canvas._pointerToTouchState = {
      activePointers: new Map(), // Key: pointerId, Value: PointerEvent
      handlers: {
        touchstart: [],
        touchmove: [],
        touchend: [],
    ...
    const state = canvas._pointerToTouchState;

    /**
     * Helper to convert a PointerEvent to a DOM Touch object.
     */
    const createTouch = (pointerEvent) => {
      // Fix for Mouse Pressure: Mouse events often report 0.5 pressure.
      // Touch logic sometimes expects 1.0 for a firm press.
      let force = pointerEvent.pressure;
      ...
      return new Touch({
        ...
        force: force,
        radiusX: pointerEvent.width / 2 || 1,
        radiusY: pointerEvent.height / 2 || 1,
      });
    };

    /**
     * Helper to fire the synthesized event to all registered handlers of that type.
     */
    const dispatchFakeEvent = (type, changedTouchesList) => {
      const handlers = state.handlers[type];
      if (!handlers || handlers.length === 0) return;

      // The 'touches' list must contain ALL currently active fingers.
      const touchesList = Array.from(state.activePointers.values()).map(
        createTouch
      );

      // The 'targetTouches' list usually matches 'touches' for full-screen canvas games.
      const targetTouchesList = touchesList;
      ...
      // Dispatch to all handlers registered for this specific event type
      handlers.forEach((func) => func(event));
    };

    // --- Attach the Master Pointer Listeners (Only once per canvas) ---
    ...
    canvas.addEventListener("pointerdown", (e) => {
      ...
      dispatchFakeEvent("touchstart", [touch]);
    });
    ...
    canvas.addEventListener("pointermove", (e) => {
      ...
      const touch = createTouch(e);
      dispatchFakeEvent("touchmove", [touch]);
    });
    ...
    canvas.addEventListener("pointerup", (e) => {
      ...
      // Remove from active map BEFORE generating 'touches', but kept for 'changedTouches'
      state.activePointers.delete(e.pointerId);

      dispatchFakeEvent("touchend", [touch]);
    });
    ...
    canvas.addEventListener("pointercancel", (e) => {
      ...
      dispatchFakeEvent("touchcancel", [touch]);
    });
  }

  // 3. Registration: Add the targetHandler to the appropriate list
  const handlerList = canvas._pointerToTouchState.handlers[eventName];
  if (handlerList) {
    // Avoid adding the same handler twice if called mistakenly
    if (!handlerList.includes(targetHandler)) {
      handlerList.push(targetHandler);
    }
  } else {
    console.warn(
      `setupPointerToTouchConversion: Unsupported event type '${eventName}'`
    );
  }
}
...
var Godot = (function () {
  ...
  return function (Godot) {
    ...
    var DYNAMIC_BASE = 7489552,
      DYNAMICTOP_PTR = 2246480;
    ...
    var JSEvents = {
      ...
      registerOrRemoveHandler: function (eventHandler) {
        if (
          ["mousemove", "mousedown", "mouseup", "wheel", "mousewheel"].includes(
        ...
        if (
          ["touchstart", "touchmove", "touchend"].includes(
            eventHandler.eventTypeString
          )
        ) {
          setupPointerToTouchConversion(
            eventHandler.target,
            jsEventHandler,
            eventHandler.eventTypeString
          );
    ...
    var ___tm_timezone = (stringToUTF8("GMT", 2246576, 4), 2246576);
    ...
    var asmLibraryArg = {
      ...
      mi: _dlopen,
      li: _dlsym,
      Pb: _eglGetProcAddress,
      ki: _emscripten_asm_const_dii,
      x: _emscripten_asm_const_iii,
      Ob: _emscripten_cancel_main_loop,
      Nb: _emscripten_enter_soft_fullscreen,
      ji: _emscripten_exit_fullscreen,
      Mb: _emscripten_exit_pointerlock,
      rb: _emscripten_exit_soft_fullscreen,
      ii: _emscripten_force_exit,
      cb: _emscripten_get_canvas_element_size,
      hi: _emscripten_get_element_css_size,
      gi: _emscripten_get_fullscreen_status,
      fi: _emscripten_get_gamepad_status,
      ei: _emscripten_get_now,
      di: _emscripten_get_num_gamepads,
      ci: _emscripten_get_pointerlock_status,
      bi: _emscripten_glActiveTexture,
      ai: _emscripten_glAttachShader,
      $h: _emscripten_glBeginQuery,
      _h: _emscripten_glBeginQueryEXT,
      Zh: _emscripten_glBeginTransformFeedback,
      Yh: _emscripten_glBindAttribLocation,
      Xh: _emscripten_glBindBuffer,
      Wh: _emscripten_glBindBufferBase,
      Vh: _emscripten_glBindBufferRange,
      Uh: _emscripten_glBindFramebuffer,
      Th: _emscripten_glBindRenderbuffer,
      Sh: _emscripten_glBindSampler,
      Rh: _emscripten_glBindTexture,
      Qh: _emscripten_glBindTransformFeedback,
      Ph: _emscripten_glBindVertexArray,
      Oh: _emscripten_glBindVertexArrayOES,
      Nh: _emscripten_glBlendColor,
      Mh: _emscripten_glBlendEquation,
      Lh: _emscripten_glBlendEquationSeparate,
      Kh: _emscripten_glBlendFunc,
      Jh: _emscripten_glBlendFuncSeparate,
      Ih: _emscripten_glBlitFramebuffer,
      Hh: _emscripten_glBufferData,
      Gh: _emscripten_glBufferSubData,
      Fh: _emscripten_glCheckFramebufferStatus,
      Eh: _emscripten_glClear,
      Dh: _emscripten_glClearBufferfi,
      Ch: _emscripten_glClearBufferfv,
      Bh: _emscripten_glClearBufferiv,
      Ah: _emscripten_glClearBufferuiv,
      zh: _emscripten_glClearColor,
      yh: _emscripten_glClearDepthf,
      xh: _emscripten_glClearStencil,
      wh: _emscripten_glClientWaitSync,
      vh: _emscripten_glColorMask,
      uh: _emscripten_glCompileShader,
      th: _emscripten_glCompressedTexImage2D,
      sh: _emscripten_glCompressedTexImage3D,
      rh: _emscripten_glCompressedTexSubImage2D,
      qh: _emscripten_glCompressedTexSubImage3D,
      ph: _emscripten_glCopyBufferSubData,
      oh: _emscripten_glCopyTexImage2D,
      nh: _emscripten_glCopyTexSubImage2D,
      mh: _emscripten_glCopyTexSubImage3D,
      ...
      dh: _emscripten_glDeleteRenderbuffers,
      ch: _emscripten_glDeleteSamplers,
      bh: _emscripten_glDeleteShader,
      ah: _emscripten_glDeleteSync,
      $g: _emscripten_glDeleteTextures,
      _g: _emscripten_glDeleteTransformFeedbacks,
      Zg: _emscripten_glDeleteVertexArrays,
      Yg: _emscripten_glDeleteVertexArraysOES,
      Xg: _emscripten_glDepthFunc,
      Wg: _emscripten_glDepthMask,
      Vg: _emscripten_glDepthRangef,
      Ug: _emscripten_glDetachShader,
      Tg: _emscripten_glDisable,
      Sg: _emscripten_glDisableVertexAttribArray,
      Rg: _emscripten_glDrawArrays,
      Qg: _emscripten_glDrawArraysInstanced,
      Pg: _emscripten_glDrawArraysInstancedANGLE,
      Og: _emscripten_glDrawArraysInstancedARB,
      Ng: _emscripten_glDrawArraysInstancedEXT,
      Mg: _emscripten_glDrawArraysInstancedNV,
      Lg: _emscripten_glDrawBuffers,
      Kg: _emscripten_glDrawBuffersEXT,
      Jg: _emscripten_glDrawBuffersWEBGL,
      Ig: _emscripten_glDrawElements,
      Hg: _emscripten_glDrawElementsInstanced,
      Gg: _emscripten_glDrawElementsInstancedANGLE,
      Fg: _emscripten_glDrawElementsInstancedARB,
      Eg: _emscripten_glDrawElementsInstancedEXT,
      Dg: _emscripten_glDrawElementsInstancedNV,
      Cg: _emscripten_glDrawRangeElements,
      Bg: _emscripten_glEnable,
      Ag: _emscripten_glEnableVertexAttribArray,
      zg: _emscripten_glEndQuery,
      yg: _emscripten_glEndQueryEXT,
      xg: _emscripten_glEndTransformFeedback,
      wg: _emscripten_glFenceSync,
      vg: _emscripten_glFinish,
      ug: _emscripten_glFlush,
      tg: _emscripten_glFlushMappedBufferRange,
      sg: _emscripten_glFramebufferRenderbuffer,
      rg: _emscripten_glFramebufferTexture2D,
      qg: _emscripten_glFramebufferTextureLayer,
      pg: _emscripten_glFrontFace,
      og: _emscripten_glGenBuffers,
      ng: _emscripten_glGenFramebuffers,
      mg: _emscripten_glGenQueries,
      lg: _emscripten_glGenQueriesEXT,
      kg: _emscripten_glGenRenderbuffers,
      jg: _emscripten_glGenSamplers,
      ig: _emscripten_glGenTextures,
      hg: _emscripten_glGenTransformFeedbacks,
      gg: _emscripten_glGenVertexArrays,
      fg: _emscripten_glGenVertexArraysOES,
      eg: _emscripten_glGenerateMipmap,
      dg: _emscripten_glGetActiveAttrib,
      cg: _emscripten_glGetActiveUniform,
      bg: _emscripten_glGetActiveUniformBlockName,
      ag: _emscripten_glGetActiveUniformBlockiv,
      $f: _emscripten_glGetActiveUniformsiv,
      _f: _emscripten_glGetAttachedShaders,
      Zf: _emscripten_glGetAttribLocation,
      Yf: _emscripten_glGetBooleanv,
      Xf: _emscripten_glGetBufferParameteri64v,
      Wf: _emscripten_glGetBufferParameteriv,
      Vf: _emscripten_glGetBufferPointerv,
      Uf: _emscripten_glGetError,
      Tf: _emscripten_glGetFloatv,
      Sf: _emscripten_glGetFragDataLocation,
      Rf: _emscripten_glGetFramebufferAttachmentParameteriv,
      Qf: _emscripten_glGetInteger64i_v,
      Pf: _emscripten_glGetInteger64v,
      Of: _emscripten_glGetIntegeri_v,
      Nf: _emscripten_glGetIntegerv,
      Mf: _emscripten_glGetInternalformativ,
      Lf: _emscripten_glGetProgramBinary,
      Kf: _emscripten_glGetProgramInfoLog,
      Jf: _emscripten_glGetProgramiv,
      If: _emscripten_glGetQueryObjecti64vEXT,
      Hf: _emscripten_glGetQueryObjectivEXT,
      Gf: _emscripten_glGetQueryObjectui64vEXT,
      Ff: _emscripten_glGetQueryObjectuiv,
      Ef: _emscripten_glGetQueryObjectuivEXT,
      Df: _emscripten_glGetQueryiv,
      Cf: _emscripten_glGetQueryivEXT,
      Bf: _emscripten_glGetRenderbufferParameteriv,
      Af: _emscripten_glGetSamplerParameterfv,
      zf: _emscripten_glGetSamplerParameteriv,
      yf: _emscripten_glGetShaderInfoLog,
      xf: _emscripten_glGetShaderPrecisionFormat,
      wf: _emscripten_glGetShaderSource,
      vf: _emscripten_glGetShaderiv,
      uf: _emscripten_glGetString,
      tf: _emscripten_glGetStringi,
      sf: _emscripten_glGetSynciv,
      rf: _emscripten_glGetTexParameterfv,
      qf: _emscripten_glGetTexParameteriv,
      pf: _emscripten_glGetTransformFeedbackVarying,
      of: _emscripten_glGetUniformBlockIndex,
      nf: _emscripten_glGetUniformIndices,
      mf: _emscripten_glGetUniformLocation,
      lf: _emscripten_glGetUniformfv,
      kf: _emscripten_glGetUniformiv,
      jf: _emscripten_glGetUniformuiv,
      hf: _emscripten_glGetVertexAttribIiv,
      gf: _emscripten_glGetVertexAttribIuiv,
      ff: _emscripten_glGetVertexAttribPointerv,
      ef: _emscripten_glGetVertexAttribfv,
      df: _emscripten_glGetVertexAttribiv,
      cf: _emscripten_glHint,
      bf: _emscripten_glInvalidateFramebuffer,
      af: _emscripten_glInvalidateSubFramebuffer,
      $e: _emscripten_glIsBuffer,
      _e: _emscripten_glIsEnabled,
      Ze: _emscripten_glIsFramebuffer,
      Ye: _emscripten_glIsProgram,
      Xe: _emscripten_glIsQuery,
      We: _emscripten_glIsQueryEXT,
      Ve: _emscripten_glIsRenderbuffer,
      Ue: _emscripten_glIsSampler,
      Te: _emscripten_glIsShader,
      Se: _emscripten_glIsSync,
      Re: _emscripten_glIsTexture,
      Qe: _emscripten_glIsTransformFeedback,
      Pe: _emscripten_glIsVertexArray,
      Oe: _emscripten_glIsVertexArrayOES,
      Ne: _emscripten_glLineWidth,
      Me: _emscripten_glLinkProgram,
      Le: _emscripten_glMapBufferRange,
      ...
      ze: _emscripten_glResumeTransformFeedback,
      ye: _emscripten_glSampleCoverage,
      xe: _emscripten_glSamplerParameterf,
      we: _emscripten_glSamplerParameterfv,
      ve: _emscripten_glSamplerParameteri,
      ue: _emscripten_glSamplerParameteriv,
      te: _emscripten_glScissor,
      se: _emscripten_glShaderBinary,
      re: _emscripten_glShaderSource,
      qe: _emscripten_glStencilFunc,
      pe: _emscripten_glStencilFuncSeparate,
      oe: _emscripten_glStencilMask,
      ne: _emscripten_glStencilMaskSeparate,
      me: _emscripten_glStencilOp,
      le: _emscripten_glStencilOpSeparate,
      ke: _emscripten_glTexImage2D,
      je: _emscripten_glTexImage3D,
      ...
      ee: _emscripten_glTexStorage2D,
      de: _emscripten_glTexStorage3D,
      ce: _emscripten_glTexSubImage2D,
      be: _emscripten_glTexSubImage3D,
      ae: _emscripten_glTransformFeedbackVaryings,
      $d: _emscripten_glUniform1f,
      _d: _emscripten_glUniform1fv,
      ...
      yd: _emscripten_glUniformMatrix3x2fv,
      xd: _emscripten_glUniformMatrix3x4fv,
      wd: _emscripten_glUniformMatrix4fv,
      vd: _emscripten_glUniformMatrix4x2fv,
      ud: _emscripten_glUniformMatrix4x3fv,
      td: _emscripten_glUnmapBuffer,
      ...
      id: _emscripten_glVertexAttribDivisor,
      hd: _emscripten_glVertexAttribDivisorANGLE,
      gd: _emscripten_glVertexAttribDivisorARB,
      fd: _emscripten_glVertexAttribDivisorEXT,
      ed: _emscripten_glVertexAttribDivisorNV,
      dd: _emscripten_glVertexAttribI4i,
      cd: _emscripten_glVertexAttribI4iv,
      bd: _emscripten_glVertexAttribI4ui,
      ad: _emscripten_glVertexAttribI4uiv,
      $c: _emscripten_glVertexAttribIPointer,
      _c: _emscripten_glVertexAttribPointer,
      Zc: _emscripten_glViewport,
      Yc: _emscripten_glWaitSync,
      K: _emscripten_longjmp,
      Xc: _emscripten_memcpy_big,
      Wc: _emscripten_pause_main_loop,
      Vc: _emscripten_request_fullscreen_strategy,
      Uc: _emscripten_request_pointerlock,
      Tc: _emscripten_resize_heap,
      Sc: _emscripten_resume_main_loop,
      Rc: _emscripten_sample_gamepad_data,
      bb: _emscripten_set_canvas_element_size,
      Qc: _emscripten_set_fullscreenchange_callback_on_thread,
      Pc: _emscripten_set_gamepadconnected_callback_on_thread,
      Oc: _emscripten_set_gamepaddisconnected_callback_on_thread,
      Nc: _emscripten_set_keydown_callback_on_thread,
      Mc: _emscripten_set_keypress_callback_on_thread,
      Lc: _emscripten_set_keyup_callback_on_thread,
      Lb: _emscripten_set_main_loop,
      Kc: _emscripten_set_mousedown_callback_on_thread,
      Jc: _emscripten_set_mousemove_callback_on_thread,
      Ic: _emscripten_set_mouseup_callback_on_thread,
      Hc: _emscripten_set_touchcancel_callback_on_thread,
      Gc: _emscripten_set_touchend_callback_on_thread,
      Fc: _emscripten_set_touchmove_callback_on_thread,
      Ec: _emscripten_set_touchstart_callback_on_thread,
      Dc: _emscripten_set_wheel_callback_on_thread,
      Kb: _emscripten_webgl_commit_frame,
      Cc: _emscripten_webgl_create_context,
      Bc: _emscripten_webgl_destroy_context,
      Ac: _emscripten_webgl_init_context_attributes,
      zc: _emscripten_webgl_make_context_current,
      qi: _environ_get,
      pi: _environ_sizes_get,
      yc: _exit,
      Ea: _fd_close,
      oi: _fd_write,
      qb: _gai_strerror,
      ...
      c: _glActiveTexture,
      ab: _glAttachShader,
      ob: _glBeginTransformFeedback,
      Ib: _glBindAttribLocation,
      d: _glBindBuffer,
      V: _glBindBufferBase,
      f: _glBindFramebuffer,
      ia: _glBindRenderbuffer,
      b: _glBindTexture,
      n: _glBindVertexArray,
      N: _glBlendEquation,
      _: _glBlendFunc,
      J: _glBlendFuncSeparate,
      ...
      T: _glColorMask,
      $a: _glCompileShader,
      Hb: _glCompressedTexImage2D,
      wc: _glCompressedTexImage3D,
      vc: _glCompressedTexSubImage2D,
      nb: _glCompressedTexSubImage3D,
      uc: _glCopyBufferSubData,
      ...
      Ra: _glDrawBuffers,
      X: _glDrawElements,
      ya: _glDrawElementsInstanced,
      B: _glEnable,
      l: _glEnableVertexAttribArray,
      lb: _glEndTransformFeedback,
      tc: _glFinish,
      ga: _glFramebufferRenderbuffer,
      F: _glFramebufferTexture2D,
      sc: _glFramebufferTextureLayer,
      Fb: _glFrontFace,
      H: _glGenBuffers,
      I: _glGenFramebuffers,
      qa: _glGenRenderbuffers,
      C: _glGenTextures,
      ...
      va: _glReadBuffer,
      kb: _glReadPixels,
      pa: _glRenderbufferStorage,
      Qa: _glRenderbufferStorageMultisample,
      W: _glScissor,
      Wa: _glShaderSource,
      A: _glTexImage2D,
      Pa: _glTexImage3D,
      ...
      Oa: _glTexSubImage2D,
      Va: _glTexSubImage3D,
      mc: _glTransformFeedbackVaryings,
      h: _glUniform1f,
      k: _glUniform1i,
      jb: _glUniform1iv,
      Cb: _glUniform1ui,
      ...
      u: _glUniform4fv,
      Ja: _glUniform4i,
      lc: _glUniformBlockBinding,
      Bb: _glUniformMatrix2fv,
      Ab: _glUniformMatrix3fv,
      g: _glUniformMatrix4fv,
      ka: _glUseProgram,
      w: _glVertexAttrib4f,
      ca: _glVertexAttrib4fv,
      E: _glVertexAttribDivisor,
      Ua: _glVertexAttribI4ui,
      Ga: _glVertexAttribIPointer,
      j: _glVertexAttribPointer,
      z: _glViewport,
      zb: _gmtime_r,
      kc: _godot_xhr_free,
      jc: _godot_xhr_get_ready_state,
      ic: _godot_xhr_get_response,
      hc: _godot_xhr_get_response_headers,
      gc: _godot_xhr_get_response_headers_length,
      fc: _godot_xhr_get_response_length,
      ec: _godot_xhr_get_status,
      dc: _godot_xhr_new,
      yb: _godot_xhr_open,
      gb: _godot_xhr_reset,
      cc: _godot_xhr_send_data,
      bc: _godot_xhr_send_string,
      ac: _godot_xhr_set_request_header,
      $b: _inet_addr,
      xb: invoke_i,
      oa: invoke_ii,
      na: invoke_iii,
