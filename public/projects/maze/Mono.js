var Godot = (function () {
  ...
  return function (Godot) {
    ...
    var STACK_MAX = 3098624,
      DYNAMIC_BASE = 8341504,
      DYNAMICTOP_PTR = 3098432;
    ...
    var ASM_CONSTS = [
      ...
      function ($0) {
        const ref = Module.IDHandler.get($0);
        return ref && ref["context"]
          ? ref["context"].destination.channelCount
          : 0;
      },
      function ($0) {
        const id = $0;
        var ref = Module.IDHandler.get(id);
        Module.async_finish.push(
          new Promise(function (accept, reject) {
            if (!ref) {
              ...
              if (ref["input"]) ref["input"].disconnect();
              ref = null;
              context
                .close()
                .then(function () {
      ...
      function () {
        ...
        ["mouseover", "mouseleave", "focus", "blur"].forEach(function (
          ...
          Module.listeners[event] = send_notification.bind(
            null,
            notifications[index]
          );
        ...
        Module.listeners["drop"] = Module.drop_handler;
        canvas.addEventListener(
          "dragover",
          Module.listeners["dragover"],
          false
        );
    ...
    function ___cxa_find_matching_catch() {
      ...
      for (var i = 0; i < typeArray.length; i++) {
        if (
          typeArray[i] &&
          ___cxa_can_catch(typeArray[i], throwntype, thrown)
        ) {
    ...
    var Browser = {
      ...
      requestAnimationFrame: function (func) {
        // Use fake request animation frame to lock framerate to 60fps for high
        // refresh rate displays.
        // In a perfect world my game logic would be framerate independent, but
        // we must make due with this world.
    ...
    var GL = {
      ...
      resizeOffscreenFramebuffer: function (context) {
        ...
        if (context.defaultDepthTarget) {
          var prevRenderBufferBinding = gl.getParameter(
        ...
        if (gl.blitFramebuffer && !context.defaultFboForbidBlitFramebuffer) {
          ...
          if (context.defaultVao) {
            ...
            for (var i = 0; i < maxVertexAttribs; ++i) {
              var prevEnabled = gl.getVertexAttrib(
                i,
    ...
    var MONO = {
      ...
      pump_message: function () {
        if (!this.mono_background_exec)
          this.mono_background_exec = Module.cwrap(
      ...
      mono_wasm_get_call_stack: function () {
        if (!this.mono_wasm_current_bp_id)
          this.mono_wasm_current_bp_id = Module.cwrap(
            "mono_wasm_current_bp_id",
            "number",
            []
          );
        if (!this.mono_wasm_enum_frames)
          this.mono_wasm_enum_frames = Module.cwrap(
            "mono_wasm_enum_frames",
            "void",
            []
          );
      ...
      mono_wasm_get_variables: function (scope, var_list) {
        if (!this.mono_wasm_get_var_info)
          this.mono_wasm_get_var_info = Module.cwrap(
      ...
      mono_wasm_get_array_values: function (objId) {
        if (!this.mono_wasm_get_array_values_info)
          this.mono_wasm_get_array_values_info = Module.cwrap(
      ...
      mono_wasm_start_single_stepping: function (kind) {
        ...
        if (!this.mono_wasm_setup_single_step)
          this.mono_wasm_setup_single_step = Module.cwrap(
            "mono_wasm_setup_single_step",
            "void",
            ["number"]
          );
      ...
      mono_wasm_set_breakpoint: function (assembly, method_token, il_offset) {
        if (!this.mono_wasm_set_bp)
          this.mono_wasm_set_bp = Module.cwrap(
            "mono_wasm_set_breakpoint",
            "number",
      ...
      mono_wasm_remove_breakpoint: function (breakpoint_id) {
        if (!this.mono_wasm_del_bp)
          this.mono_wasm_del_bp = Module.cwrap(
            "mono_wasm_remove_breakpoint",
            "number",
            ["number"]
          );
      ...
      mono_wasm_setenv: function (name, value) {
        if (!this.wasm_setenv)
          this.wasm_setenv = Module.cwrap("mono_wasm_setenv", "void", [
            "string",
            "string",
          ]);
      ...
      mono_wasm_set_runtime_options: function (options) {
        if (!this.wasm_parse_runtime_options)
          this.wasm_parse_runtime_options = Module.cwrap(
            "mono_wasm_parse_runtime_options",
            "void",
            ["number", "number"]
          );
        var argv = Module._malloc(options.length * 4);
        var wasm_strdup = Module.cwrap("mono_wasm_strdup", "number", [
          "string",
        ]);
        aindex = 0;
      ...
      mono_load_runtime_and_bcl: function (
        ...
        var loaded_files = [];
        var mono_wasm_add_assembly = Module.cwrap(
        ...
        file_list.forEach(function (file_name) {
          var fetch_promise = fetch_file_cb(
            locateFile(deploy_prefix + "/" + file_name)
          );
          fetch_promise
            .then(function (response) {
              if (!response.ok) throw "failed to load '" + file_name + "'";
              ...
              if (pending == 0) {
                ...
                if (ENVIRONMENT_IS_SHELL) {
                  try {
                    ...
                    print("Stacktrace: \n");
                    print(err.stack);
                    var wasm_exit = Module.cwrap("mono_wasm_exit", "void", [
                      "number",
                    ]);
                    wasm_exit(1);
                  }
      ...
      mono_wasm_clear_all_breakpoints: function () {
        if (this.mono_clear_bps)
          this.mono_clear_bps = Module.cwrap(
            "mono_wasm_clear_all_breakpoints",
            "void",
            []
          );
        this.mono_clear_bps();
    ...
    function _mono_set_timeout(timeout, id) {
      if (!this.mono_set_timeout_exec)
        this.mono_set_timeout_exec = Module.cwrap(
    ...
    function _mono_wasm_add_string_var(var_value) {
      if (var_value == 0) {
        ...
        MONO.var_info.push({
          value: { type: "string", value: Module.UTF8ToString(var_value) },
        });
      }
    }
    var BINDING = {
      ...
      export_functions: function (module) {
        ...
        module["mono_method_resolve"] =
          BINDING.resolve_method_fqn.bind(BINDING);
        module["mono_bind_static_method"] =
          BINDING.bind_static_method.bind(BINDING);
        module["mono_call_static_method"] =
          BINDING.call_static_method.bind(BINDING);
      },
      bindings_lazy_init: function () {
        if (this.init) return;
        this.assembly_load = Module.cwrap("mono_wasm_assembly_load", "number", [
          "string",
        ]);
        this.find_class = Module.cwrap(
          "mono_wasm_assembly_find_class",
          "number",
          ["number", "string", "string"]
        );
        this.find_method = Module.cwrap(
        ...
        this.mono_string_get_utf8 = Module.cwrap(
          "mono_wasm_string_get_utf8",
          "number",
          ["number"]
        );
        this.js_string_to_mono_string = Module.cwrap(
          "mono_wasm_string_from_js",
          "number",
          ["string"]
        );
        this.mono_get_obj_type = Module.cwrap(
          "mono_wasm_get_obj_type",
          "number",
          ["number"]
        );
        this.mono_unbox_int = Module.cwrap("mono_unbox_int", "number", [
          "number",
        ]);
        this.mono_unbox_float = Module.cwrap(
          "mono_wasm_unbox_float",
          "number",
          ["number"]
        );
        this.mono_array_length = Module.cwrap(
          "mono_wasm_array_length",
          "number",
          ["number"]
        );
        this.mono_array_get = Module.cwrap("mono_wasm_array_get", "number", [
          "number",
          "number",
        ]);
        this.mono_obj_array_new = Module.cwrap(
          "mono_wasm_obj_array_new",
          "number",
          ["number"]
        );
        this.mono_obj_array_set = Module.cwrap(
          "mono_wasm_obj_array_set",
          "void",
          ["number", "number", "number"]
        );
        this.mono_unbox_enum = Module.cwrap("mono_wasm_unbox_enum", "number", [
          "number",
        ]);
        this.mono_typed_array_new = Module.cwrap(
          "mono_wasm_typed_array_new",
          "number",
          ["number", "number", "number", "number"]
        );
        ...
        var binding_fqn_class = this.BINDING_ASM.substring(
          this.BINDING_ASM.indexOf("]") + 1
        ).trim();
        ...
        if (
          binding_fqn_class !== null &&
          typeof binding_fqn_class !== "undefined"
        ) {
          var namespace = "WebAssembly";
          var classname =
            binding_fqn_class.length > 0 ? binding_fqn_class : "Runtime";
        ...
        var wasm_runtime_class = this.find_class(
          this.binding_module,
          namespace,
          classname
        );
      ...
      js_array_to_mono_array: function (js_array) {
        ...
        for (var i = 0; i < js_array.length; ++i) {
          this.mono_obj_array_set(
            mono_array,
            i,
            this.js_to_mono_obj(js_array[i])
          );
        }
        return mono_array;
      },
      unbox_mono_obj: function (mono_obj) {
        ...
        switch (type) {
          ...
          case 6: {
            ...
            this.call_method(this.setup_js_cont, null, "mo", [
              mono_obj,
              cont_obj,
            ]);
          ...
          case 9:
            if (this.mono_wasm_marshal_enum_as_int) {
              ...
              enumValue = this.call_method(this.object_to_string, null, "m", [
                mono_obj,
              ]);
            }
            return enumValue;
          ...
          case 20:
            var dateValue = this.call_method(this.get_date_value, null, "md", [
              mono_obj,
            ]);
            return new Date(dateValue);
          case 21:
            var dateoffsetValue = this.call_method(
      ...
      set_task_failure: function (tcs, reason) {
        tcs.is_mono_tcs_result_set = true;
        this.call_method(this.set_tcs_failure, null, "os", [
          tcs,
          reason.toString(),
        ]);
        if (tcs.is_mono_tcs_task_bound) this.free_task_completion_source(tcs);
      ...
      js_typedarray_to_heap: function (typedArray) {
        ...
        heapBytes.set(
          new Uint8Array(typedArray.buffer, typedArray.byteOffset, numBytes)
        );
        return heapBytes;
      },
      js_to_mono_obj: function (js_obj) {
        ...
        if (js_obj.constructor.name === "Date")
          return this.call_method(this.create_date_time, null, "dm", [
            js_obj.getTime(),
          ]);
      ...
      js_typed_array_to_array: function (js_obj) {
        if (
          !!(js_obj.buffer instanceof ArrayBuffer && js_obj.BYTES_PER_ELEMENT)
        ) {
          var arrayType = 0;
      ...
      typedarray_copy_to: function (
        ...
        if (
          !!(
            typed_array.buffer instanceof ArrayBuffer &&
      ...
      typedarray_copy_from: function (
        ...
        if (
          ...
          var view_bytes = typed_array.length * typed_array.BYTES_PER_ELEMENT;
          if (num_of_bytes > view_bytes) num_of_bytes = view_bytes;
          var typedarrayBytes = new Uint8Array(
            typed_array.buffer,
            0,
            num_of_bytes
          );
          ...
          typedarrayBytes.set(
            Module.HEAPU8.subarray(
              pinned_array + offset,
              pinned_array + offset + num_of_bytes
            )
          );
          return num_of_bytes;
        } else {
      ...
      wasm_binding_obj_new: function (js_obj_id, type) {
        return this.call_method(this.bind_js_obj, null, "io", [
          js_obj_id,
          type,
        ]);
      },
      wasm_bind_existing: function (mono_obj, js_id) {
        return this.call_method(this.bind_existing_obj, null, "mi", [
          mono_obj,
          js_id,
        ]);
      },
      wasm_bind_core_clr_obj: function (js_id, gc_handle) {
        return this.call_method(this.bind_core_clr_obj, null, "ii", [
          js_id,
          gc_handle,
        ]);
      },
      ...
      get_task_and_bind: function (tcs, js_obj) {
        ...
        var task_gchandle = this.call_method(
          this.tcs_get_task_and_bind,
          null,
          "oi",
          [tcs, gc_handle + 1]
        );
      ...
      free_task_completion_source: function (tcs) {
        if (tcs.is_mono_tcs_result_set) {
          this.call_method(this.unbind_raw_obj_and_free, null, "ii", [
            tcs.__mono_gchandle__,
          ]);
        }
        if (tcs.__mono_bound_task__) {
          this.call_method(this.unbind_raw_obj_and_free, null, "ii", [
            tcs.__mono_bound_task__,
          ]);
        }
      },
      ...
      call_method: function (method, this_arg, args_marshal, args) {
        ...
        var extra_args_mem = extra_args_mem
          ? Module._malloc(extra_args_mem)
          : 0;
        var extra_arg_idx = 0;
        ...
        for (var i = 0; i < args.length; ++i) {
          if (args_marshal[i] == "s") {
            Module.setValue(
              args_mem + i * 4,
              this.js_string_to_mono_string(args[i]),
              "i32"
            );
        ...
        if (args_marshal !== null && typeof args_marshal !== "undefined") {
          if (
            args_marshal.length >= args.length &&
            args_marshal[args.length] === "m"
          )
            return res;
        }
      ...
      invoke_delegate: function (delegate_obj, js_args) {
        ...
        if (!this.delegate_dynamic_invoke) {
          ...
          if (!this.delegate_class)
            this.delegate_class = this.find_class(
              this.corlib,
              "System",
              "Delegate"
            );
          ...
          this.delegate_dynamic_invoke = this.find_method(
            this.delegate_class,
            "DynamicInvoke",
            -1
          );
        ...
        if (!this.delegate_dynamic_invoke)
          throw new Error(
            "System.Delegate.DynamicInvoke method can not be resolved."
          );
      ...
      resolve_method_fqn: function (fqn) {
        var assembly = fqn
          .substring(fqn.indexOf("[") + 1, fqn.indexOf("]"))
          .trim();
        ...
        if (!klass)
          throw new Error(
            "Could not find class: " + namespace + ":" + classname
          );
      ...
      mono_wasm_register_obj: function (obj) {
        ...
        if (obj !== null && typeof obj !== "undefined") {
          ...
          if (typeof gc_handle === "undefined") {
            ...
            gc_handle = obj.__mono_gchandle__ = this.wasm_binding_obj_new(
              handle + 1,
              wasm_type
            );
      ...
      mono_wasm_unregister_obj: function (js_id) {
        ...
        if (typeof obj !== "undefined" && obj !== null) {
          if (
            typeof ___mono_wasm_global___ !== "undefined" &&
            ___mono_wasm_global___ === obj
          )
            return obj;
      ...
      mono_wasm_get_global: function () {
        function testGlobal(obj) {
          ...
          var success =
            typeof ___mono_wasm_global___ === "object" &&
            obj["___mono_wasm_global___"] === obj;
          if (!success) {
    ...
    function _mono_wasm_bind_core_object(js_handle, gc_handle, is_exception) {
      ...
      if (!requireObject) {
        ...
        return BINDING.js_string_to_mono_string(
          "Invalid JS object handle '" + js_handle + "'"
        );
      }
    ...
    function _mono_wasm_get_global_object(global_name, is_exception) {
      ...
      if (globalObj === null || typeof globalObj === undefined) {
        setValue(is_exception, 1, "i32");
        return BINDING.js_string_to_mono_string(
          "Global object '" + js_name + "' not found."
        );
      }
    ...
    function _mono_wasm_get_object_property(
      ...
      if (!js_name) {
        setValue(is_exception, 1, "i32");
        return BINDING.js_string_to_mono_string(
          "Invalid property name object '" + js_name + "'"
        );
      }
      var res;
      try {
    ...
    var DOTNET = {
      _dotnet_get_global: function () {
        function testGlobal(obj) {
          ...
          var success =
            typeof ___dotnet_global___ === "object" &&
            obj["___dotnet_global___"] === obj;
          if (!success) {
    ...
    function _mono_wasm_invoke_js_unmarshalled(
      exceptionMessage,
      funcName,
      arg0,
      arg1,
      ...
      try {
        ...
        var exceptionJsString = ex.message + "\n" + ex.stack;
        var mono_string = Module.cwrap("mono_wasm_string_from_js", "number", [
          "string",
        ]);
        var exceptionSystemString = mono_string(exceptionJsString);
        setValue(exceptionMessage, exceptionSystemString, "i32");
    ...
    function _mono_wasm_invoke_js_with_args(
      ...
      if (!js_name) {
        setValue(is_exception, 1, "i32");
        return BINDING.js_string_to_mono_string(
          "Invalid method name object '" + method_name + "'"
        );
      }
    ...
    function _mono_wasm_new_object(
      ...
      if (!object_handle_or_function) {
        ...
        if (typeof object_handle_or_function === "function")
          ...
          requireObject = BINDING.mono_wasm_require_handle(
            object_handle_or_function
          );
        if (!requireObject) {
          setValue(is_exception, 1, "i32");
    ...
    function _mono_wasm_set_by_index(
      js_handle,
      property_index,
      value,
      is_exception
    ...
    function _mono_wasm_typed_array_from(
      ...
      BINDING.bindings_lazy_init();
      var res = BINDING.typed_array_from(
        pinned_array,
        begin,
        end,
        bytes_per_element,
    ...
    var asmLibraryArg = {
      a: ___assert_fail,
      kk: ___clock_gettime,
      jk: ___cxa_allocate_exception,
      s: ___cxa_atexit,
      ik: ___cxa_begin_catch,
      hk: ___cxa_end_catch,
      gk: ___cxa_find_matching_catch_3,
      fk: ___cxa_throw,
      Ua: ___lock,
      ek: ___map_file,
      dk: ___resumeException,
      ck: ___syscall10,
      na: ___syscall102,
      bk: ___syscall118,
      ak: ___syscall12,
      ...
      mj: _dlopen,
      lj: _dlsym,
      qc: _eglGetProcAddress,
      kj: _emscripten_asm_const_dii,
      w: _emscripten_asm_const_iii,
      pc: _emscripten_cancel_main_loop,
      oc: _emscripten_enter_soft_fullscreen,
      jj: _emscripten_exit_fullscreen,
      nc: _emscripten_exit_pointerlock,
      Sb: _emscripten_exit_soft_fullscreen,
      ij: _emscripten_force_exit,
      Db: _emscripten_get_canvas_element_size,
      hj: _emscripten_get_element_css_size,
      gj: _emscripten_get_fullscreen_status,
      fj: _emscripten_get_gamepad_status,
      ej: _emscripten_get_now,
      dj: _emscripten_get_num_gamepads,
      cj: _emscripten_get_pointerlock_status,
      bj: _emscripten_glActiveTexture,
      aj: _emscripten_glAttachShader,
      $i: _emscripten_glBeginQuery,
      _i: _emscripten_glBeginQueryEXT,
      Zi: _emscripten_glBeginTransformFeedback,
      Yi: _emscripten_glBindAttribLocation,
      Xi: _emscripten_glBindBuffer,
      Wi: _emscripten_glBindBufferBase,
      Vi: _emscripten_glBindBufferRange,
      Ui: _emscripten_glBindFramebuffer,
      Ti: _emscripten_glBindRenderbuffer,
      Si: _emscripten_glBindSampler,
      Ri: _emscripten_glBindTexture,
      Qi: _emscripten_glBindTransformFeedback,
      Pi: _emscripten_glBindVertexArray,
      Oi: _emscripten_glBindVertexArrayOES,
      Ni: _emscripten_glBlendColor,
      Mi: _emscripten_glBlendEquation,
      Li: _emscripten_glBlendEquationSeparate,
      Ki: _emscripten_glBlendFunc,
      Ji: _emscripten_glBlendFuncSeparate,
      Ii: _emscripten_glBlitFramebuffer,
      Hi: _emscripten_glBufferData,
      Gi: _emscripten_glBufferSubData,
      Fi: _emscripten_glCheckFramebufferStatus,
      Ei: _emscripten_glClear,
      Di: _emscripten_glClearBufferfi,
      Ci: _emscripten_glClearBufferfv,
      Bi: _emscripten_glClearBufferiv,
      Ai: _emscripten_glClearBufferuiv,
      zi: _emscripten_glClearColor,
      yi: _emscripten_glClearDepthf,
      xi: _emscripten_glClearStencil,
      wi: _emscripten_glClientWaitSync,
      vi: _emscripten_glColorMask,
      ui: _emscripten_glCompileShader,
      ti: _emscripten_glCompressedTexImage2D,
      si: _emscripten_glCompressedTexImage3D,
      ri: _emscripten_glCompressedTexSubImage2D,
      qi: _emscripten_glCompressedTexSubImage3D,
      pi: _emscripten_glCopyBufferSubData,
      oi: _emscripten_glCopyTexImage2D,
      ni: _emscripten_glCopyTexSubImage2D,
      mi: _emscripten_glCopyTexSubImage3D,
      li: _emscripten_glCreateProgram,
      ki: _emscripten_glCreateShader,
      ji: _emscripten_glCullFace,
      ii: _emscripten_glDeleteBuffers,
      hi: _emscripten_glDeleteFramebuffers,
      gi: _emscripten_glDeleteProgram,
      fi: _emscripten_glDeleteQueries,
      ei: _emscripten_glDeleteQueriesEXT,
      di: _emscripten_glDeleteRenderbuffers,
      ci: _emscripten_glDeleteSamplers,
      bi: _emscripten_glDeleteShader,
      ai: _emscripten_glDeleteSync,
      $h: _emscripten_glDeleteTextures,
      _h: _emscripten_glDeleteTransformFeedbacks,
      Zh: _emscripten_glDeleteVertexArrays,
      Yh: _emscripten_glDeleteVertexArraysOES,
      Xh: _emscripten_glDepthFunc,
      Wh: _emscripten_glDepthMask,
      Vh: _emscripten_glDepthRangef,
      Uh: _emscripten_glDetachShader,
      Th: _emscripten_glDisable,
      Sh: _emscripten_glDisableVertexAttribArray,
      Rh: _emscripten_glDrawArrays,
      Qh: _emscripten_glDrawArraysInstanced,
      Ph: _emscripten_glDrawArraysInstancedANGLE,
      Oh: _emscripten_glDrawArraysInstancedARB,
      Nh: _emscripten_glDrawArraysInstancedEXT,
      Mh: _emscripten_glDrawArraysInstancedNV,
      Lh: _emscripten_glDrawBuffers,
      Kh: _emscripten_glDrawBuffersEXT,
      Jh: _emscripten_glDrawBuffersWEBGL,
      Ih: _emscripten_glDrawElements,
      Hh: _emscripten_glDrawElementsInstanced,
      Gh: _emscripten_glDrawElementsInstancedANGLE,
      Fh: _emscripten_glDrawElementsInstancedARB,
      Eh: _emscripten_glDrawElementsInstancedEXT,
      Dh: _emscripten_glDrawElementsInstancedNV,
      Ch: _emscripten_glDrawRangeElements,
      Bh: _emscripten_glEnable,
      Ah: _emscripten_glEnableVertexAttribArray,
      zh: _emscripten_glEndQuery,
      yh: _emscripten_glEndQueryEXT,
      xh: _emscripten_glEndTransformFeedback,
      wh: _emscripten_glFenceSync,
      vh: _emscripten_glFinish,
      uh: _emscripten_glFlush,
      th: _emscripten_glFlushMappedBufferRange,
      sh: _emscripten_glFramebufferRenderbuffer,
      rh: _emscripten_glFramebufferTexture2D,
      qh: _emscripten_glFramebufferTextureLayer,
      ph: _emscripten_glFrontFace,
      oh: _emscripten_glGenBuffers,
      nh: _emscripten_glGenFramebuffers,
      mh: _emscripten_glGenQueries,
      lh: _emscripten_glGenQueriesEXT,
      kh: _emscripten_glGenRenderbuffers,
      jh: _emscripten_glGenSamplers,
      ih: _emscripten_glGenTextures,
      hh: _emscripten_glGenTransformFeedbacks,
      gh: _emscripten_glGenVertexArrays,
      fh: _emscripten_glGenVertexArraysOES,
      eh: _emscripten_glGenerateMipmap,
      dh: _emscripten_glGetActiveAttrib,
      ch: _emscripten_glGetActiveUniform,
      bh: _emscripten_glGetActiveUniformBlockName,
      ah: _emscripten_glGetActiveUniformBlockiv,
      $g: _emscripten_glGetActiveUniformsiv,
      _g: _emscripten_glGetAttachedShaders,
      Zg: _emscripten_glGetAttribLocation,
      Yg: _emscripten_glGetBooleanv,
      Xg: _emscripten_glGetBufferParameteri64v,
      Wg: _emscripten_glGetBufferParameteriv,
      Vg: _emscripten_glGetBufferPointerv,
      Ug: _emscripten_glGetError,
      Tg: _emscripten_glGetFloatv,
      Sg: _emscripten_glGetFragDataLocation,
      Rg: _emscripten_glGetFramebufferAttachmentParameteriv,
      Qg: _emscripten_glGetInteger64i_v,
      Pg: _emscripten_glGetInteger64v,
      Og: _emscripten_glGetIntegeri_v,
      Ng: _emscripten_glGetIntegerv,
      Mg: _emscripten_glGetInternalformativ,
      Lg: _emscripten_glGetProgramBinary,
      Kg: _emscripten_glGetProgramInfoLog,
      Jg: _emscripten_glGetProgramiv,
      Ig: _emscripten_glGetQueryObjecti64vEXT,
      Hg: _emscripten_glGetQueryObjectivEXT,
      Gg: _emscripten_glGetQueryObjectui64vEXT,
      Fg: _emscripten_glGetQueryObjectuiv,
      Eg: _emscripten_glGetQueryObjectuivEXT,
      Dg: _emscripten_glGetQueryiv,
      Cg: _emscripten_glGetQueryivEXT,
      Bg: _emscripten_glGetRenderbufferParameteriv,
      Ag: _emscripten_glGetSamplerParameterfv,
      zg: _emscripten_glGetSamplerParameteriv,
      yg: _emscripten_glGetShaderInfoLog,
      xg: _emscripten_glGetShaderPrecisionFormat,
      wg: _emscripten_glGetShaderSource,
      vg: _emscripten_glGetShaderiv,
      ug: _emscripten_glGetString,
      tg: _emscripten_glGetStringi,
      sg: _emscripten_glGetSynciv,
      rg: _emscripten_glGetTexParameterfv,
      qg: _emscripten_glGetTexParameteriv,
      pg: _emscripten_glGetTransformFeedbackVarying,
      og: _emscripten_glGetUniformBlockIndex,
      ng: _emscripten_glGetUniformIndices,
      mg: _emscripten_glGetUniformLocation,
      lg: _emscripten_glGetUniformfv,
      kg: _emscripten_glGetUniformiv,
      jg: _emscripten_glGetUniformuiv,
      ig: _emscripten_glGetVertexAttribIiv,
      hg: _emscripten_glGetVertexAttribIuiv,
      gg: _emscripten_glGetVertexAttribPointerv,
      fg: _emscripten_glGetVertexAttribfv,
      eg: _emscripten_glGetVertexAttribiv,
      dg: _emscripten_glHint,
      cg: _emscripten_glInvalidateFramebuffer,
      bg: _emscripten_glInvalidateSubFramebuffer,
      ag: _emscripten_glIsBuffer,
      $f: _emscripten_glIsEnabled,
      _f: _emscripten_glIsFramebuffer,
      Zf: _emscripten_glIsProgram,
      Yf: _emscripten_glIsQuery,
      Xf: _emscripten_glIsQueryEXT,
      Wf: _emscripten_glIsRenderbuffer,
      Vf: _emscripten_glIsSampler,
      Uf: _emscripten_glIsShader,
      Tf: _emscripten_glIsSync,
      Sf: _emscripten_glIsTexture,
      Rf: _emscripten_glIsTransformFeedback,
      Qf: _emscripten_glIsVertexArray,
      Pf: _emscripten_glIsVertexArrayOES,
      Of: _emscripten_glLineWidth,
      Nf: _emscripten_glLinkProgram,
      Mf: _emscripten_glMapBufferRange,
      Lf: _emscripten_glPauseTransformFeedback,
      Kf: _emscripten_glPixelStorei,
      Jf: _emscripten_glPolygonOffset,
      If: _emscripten_glProgramBinary,
      Hf: _emscripten_glProgramParameteri,
      Gf: _emscripten_glQueryCounterEXT,
      Ff: _emscripten_glReadBuffer,
      Ef: _emscripten_glReadPixels,
      Df: _emscripten_glReleaseShaderCompiler,
      Cf: _emscripten_glRenderbufferStorage,
      Bf: _emscripten_glRenderbufferStorageMultisample,
      Af: _emscripten_glResumeTransformFeedback,
      zf: _emscripten_glSampleCoverage,
      yf: _emscripten_glSamplerParameterf,
      xf: _emscripten_glSamplerParameterfv,
      wf: _emscripten_glSamplerParameteri,
      vf: _emscripten_glSamplerParameteriv,
      uf: _emscripten_glScissor,
      tf: _emscripten_glShaderBinary,
      sf: _emscripten_glShaderSource,
      rf: _emscripten_glStencilFunc,
      qf: _emscripten_glStencilFuncSeparate,
      pf: _emscripten_glStencilMask,
      of: _emscripten_glStencilMaskSeparate,
      nf: _emscripten_glStencilOp,
      mf: _emscripten_glStencilOpSeparate,
      lf: _emscripten_glTexImage2D,
      kf: _emscripten_glTexImage3D,
      jf: _emscripten_glTexParameterf,
      hf: _emscripten_glTexParameterfv,
      gf: _emscripten_glTexParameteri,
      ff: _emscripten_glTexParameteriv,
      ef: _emscripten_glTexStorage2D,
      df: _emscripten_glTexStorage3D,
      cf: _emscripten_glTexSubImage2D,
      bf: _emscripten_glTexSubImage3D,
      af: _emscripten_glTransformFeedbackVaryings,
      $e: _emscripten_glUniform1f,
      _e: _emscripten_glUniform1fv,
      Ze: _emscripten_glUniform1i,
      Ye: _emscripten_glUniform1iv,
      Xe: _emscripten_glUniform1ui,
      We: _emscripten_glUniform1uiv,
      Ve: _emscripten_glUniform2f,
      Ue: _emscripten_glUniform2fv,
      Te: _emscripten_glUniform2i,
      Se: _emscripten_glUniform2iv,
      Re: _emscripten_glUniform2ui,
      Qe: _emscripten_glUniform2uiv,
      Pe: _emscripten_glUniform3f,
      Oe: _emscripten_glUniform3fv,
      Ne: _emscripten_glUniform3i,
      Me: _emscripten_glUniform3iv,
      Le: _emscripten_glUniform3ui,
      Ke: _emscripten_glUniform3uiv,
      Je: _emscripten_glUniform4f,
      Ie: _emscripten_glUniform4fv,
      He: _emscripten_glUniform4i,
      Ge: _emscripten_glUniform4iv,
      Fe: _emscripten_glUniform4ui,
      Ee: _emscripten_glUniform4uiv,
      De: _emscripten_glUniformBlockBinding,
      Ce: _emscripten_glUniformMatrix2fv,
      Be: _emscripten_glUniformMatrix2x3fv,
      ...
      we: _emscripten_glUniformMatrix4fv,
      ve: _emscripten_glUniformMatrix4x2fv,
      ue: _emscripten_glUniformMatrix4x3fv,
      te: _emscripten_glUnmapBuffer,
      se: _emscripten_glUseProgram,
      re: _emscripten_glValidateProgram,
      qe: _emscripten_glVertexAttrib1f,
      pe: _emscripten_glVertexAttrib1fv,
      oe: _emscripten_glVertexAttrib2f,
      ne: _emscripten_glVertexAttrib2fv,
      me: _emscripten_glVertexAttrib3f,
      le: _emscripten_glVertexAttrib3fv,
      ke: _emscripten_glVertexAttrib4f,
      je: _emscripten_glVertexAttrib4fv,
      ie: _emscripten_glVertexAttribDivisor,
      he: _emscripten_glVertexAttribDivisorANGLE,
      ge: _emscripten_glVertexAttribDivisorARB,
      fe: _emscripten_glVertexAttribDivisorEXT,
      ee: _emscripten_glVertexAttribDivisorNV,
      de: _emscripten_glVertexAttribI4i,
      ce: _emscripten_glVertexAttribI4iv,
      be: _emscripten_glVertexAttribI4ui,
      ae: _emscripten_glVertexAttribI4uiv,
      $d: _emscripten_glVertexAttribIPointer,
      _d: _emscripten_glVertexAttribPointer,
      Zd: _emscripten_glViewport,
      Yd: _emscripten_glWaitSync,
      P: _emscripten_longjmp,
      Xd: _emscripten_memcpy_big,
      Wd: _emscripten_pause_main_loop,
      Vd: _emscripten_request_fullscreen_strategy,
      Ud: _emscripten_request_pointerlock,
      Td: _emscripten_resize_heap,
      Sd: _emscripten_resume_main_loop,
      Rd: _emscripten_sample_gamepad_data,
      Cb: _emscripten_set_canvas_element_size,
      Qd: _emscripten_set_fullscreenchange_callback_on_thread,
      Pd: _emscripten_set_gamepadconnected_callback_on_thread,
      Od: _emscripten_set_gamepaddisconnected_callback_on_thread,
      Nd: _emscripten_set_keydown_callback_on_thread,
      Md: _emscripten_set_keypress_callback_on_thread,
      Ld: _emscripten_set_keyup_callback_on_thread,
      mc: _emscripten_set_main_loop,
      Kd: _emscripten_set_mousedown_callback_on_thread,
      Jd: _emscripten_set_mousemove_callback_on_thread,
      Id: _emscripten_set_mouseup_callback_on_thread,
      Hd: _emscripten_set_touchcancel_callback_on_thread,
      Gd: _emscripten_set_touchend_callback_on_thread,
      Fd: _emscripten_set_touchmove_callback_on_thread,
      Ed: _emscripten_set_touchstart_callback_on_thread,
      Dd: _emscripten_set_wheel_callback_on_thread,
      lc: _emscripten_webgl_commit_frame,
      Cd: _emscripten_webgl_create_context,
      Bd: _emscripten_webgl_destroy_context,
      Ad: _emscripten_webgl_init_context_attributes,
      zd: _emscripten_webgl_make_context_current,
      sj: _environ_get,
      rj: _environ_sizes_get,
      ia: _exit,
      Ta: _fd_close,
      qj: _fd_write,
      yd: _fork,
      ...
      c: _glActiveTexture,
      zb: _glAttachShader,
      Qb: _glBeginTransformFeedback,
      jc: _glBindAttribLocation,
      d: _glBindBuffer,
      ...
      jb: _glTexSubImage2D,
      sb: _glTexSubImage3D,
      md: _glTransformFeedbackVaryings,
      h: _glUniform1f,
      k: _glUniform1i,
      Lb: _glUniform1iv,
      dc: _glUniform1ui,
      ...
      u: _glUniform4fv,
      ab: _glUniform4i,
      ld: _glUniformBlockBinding,
      cc: _glUniformMatrix2fv,
      bc: _glUniformMatrix3fv,
      g: _glUniformMatrix4fv,
      xa: _glUseProgram,
      x: _glVertexAttrib4f,
      oa: _glVertexAttrib4fv,
      E: _glVertexAttribDivisor,
      rb: _glVertexAttribI4ui,
      Wa: _glVertexAttribIPointer,
      j: _glVertexAttribPointer,
      z: _glViewport,
      Ib: _gmtime_r,
      kd: _godot_xhr_free,
      jd: _godot_xhr_get_ready_state,
      id: _godot_xhr_get_response,
      hd: _godot_xhr_get_response_headers,
      gd: _godot_xhr_get_response_headers_length,
      fd: _godot_xhr_get_response_length,
      ed: _godot_xhr_get_status,
      dd: _godot_xhr_new,
      ac: _godot_xhr_open,
      Hb: _godot_xhr_reset,
      cd: _godot_xhr_send_data,
      bd: _godot_xhr_send_string,
      ad: _godot_xhr_set_request_header,
      $c: _inet_addr,
      $b: invoke_i,
      Ea: invoke_ii,
      Da: invoke_iii,
      ...
      Zb: invoke_viiiiiii,
      Yb: _kill,
      Vc: _llvm_eh_typeid_for,
      W: _localtime_r,
      memory: wasmMemory,
      Uc: _mono_set_timeout,
      Tc: _mono_wasm_add_array_item,
      Xc: _mono_wasm_add_array_var,
      Sc: _mono_wasm_add_bool_var,
      Rc: _mono_wasm_add_frame,
      Ia: _mono_wasm_add_number_var,
      Wc: _mono_wasm_add_obj_var,
      Fb: _mono_wasm_add_properties_var,
      qb: _mono_wasm_add_string_var,
      Qc: _mono_wasm_bind_core_object,
      Pc: _mono_wasm_bind_host_object,
      Oc: _mono_wasm_fire_bp,
      Nc: _mono_wasm_get_by_index,
      Mc: _mono_wasm_get_global_object,
      Lc: _mono_wasm_get_object_property,
      Kc: _mono_wasm_invoke_js_marshalled,
      Jc: _mono_wasm_invoke_js_unmarshalled,
      Ic: _mono_wasm_invoke_js_with_args,
      Hc: _mono_wasm_new,
      Gc: _mono_wasm_new_object,
      Fc: _mono_wasm_release_handle,
      Ec: _mono_wasm_release_object,
      Dc: _mono_wasm_set_by_index,
      Cc: _mono_wasm_set_object_property,
      Bc: _mono_wasm_typed_array_copy_from,
      Ac: _mono_wasm_typed_array_copy_to,
      zc: _mono_wasm_typed_array_from,
      yc: _mono_wasm_typed_array_to_array,
      $a: _nanosleep,
      ma: _pthread_cleanup_pop,
      la: _pthread_cleanup_push,
      _a: _pthread_cond_destroy,
      Ba: _pthread_cond_init,
      ...
      Za: _pthread_setcancelstate,
      hb: _round,
      ka: _saveSetjmp,
      wc: _schedule_background_exec,
      gb: _sem_destroy,
      fb: _sem_init,
      eb: _sem_post,
      Eb: _sem_trywait,
    ...
    var _mono_wasm_setup_single_step = (Module["_mono_wasm_setup_single_step"] =
      function () {
        return Module["asm"]["ok"].apply(null, arguments);
      });
    var _mono_wasm_clear_all_breakpoints = (Module[
      "_mono_wasm_clear_all_breakpoints"
    ] = function () {
      return Module["asm"]["pk"].apply(null, arguments);
    });
    var _mono_wasm_set_breakpoint = (Module["_mono_wasm_set_breakpoint"] =
      function () {
        return Module["asm"]["qk"].apply(null, arguments);
      });
    var _mono_wasm_remove_breakpoint = (Module["_mono_wasm_remove_breakpoint"] =
      function () {
        return Module["asm"]["rk"].apply(null, arguments);
      });
    var _mono_wasm_current_bp_id = (Module["_mono_wasm_current_bp_id"] =
      function () {
        return Module["asm"]["sk"].apply(null, arguments);
      });
    var _mono_wasm_enum_frames = (Module["_mono_wasm_enum_frames"] =
      function () {
        return Module["asm"]["tk"].apply(null, arguments);
      });
    var _mono_wasm_get_var_info = (Module["_mono_wasm_get_var_info"] =
      function () {
        return Module["asm"]["uk"].apply(null, arguments);
      });
    var _mono_wasm_get_object_properties = (Module[
      "_mono_wasm_get_object_properties"
    ] = function () {
      return Module["asm"]["vk"].apply(null, arguments);
    });
    var _mono_wasm_get_array_values = (Module["_mono_wasm_get_array_values"] =
      function () {
        return Module["asm"]["wk"].apply(null, arguments);
      });
    var _mono_set_timeout_exec = (Module["_mono_set_timeout_exec"] =
      function () {
    ...
    var __drop_files_callback = (Module["__drop_files_callback"] = function () {
      return Module["asm"]["Jk"].apply(null, arguments);
    });
    var _cleanup_after_sync = (Module["_cleanup_after_sync"] = function () {
    ...
    var _mono_wasm_assembly_load = (Module["_mono_wasm_assembly_load"] =
      function () {
        return Module["asm"]["Sk"].apply(null, arguments);
      });
    ...
    var _mono_wasm_invoke_method = (Module["_mono_wasm_invoke_method"] =
      function () {
        return Module["asm"]["Vk"].apply(null, arguments);
      });
    var _mono_wasm_assembly_get_entry_point = (Module[
      "_mono_wasm_assembly_get_entry_point"
    ] = function () {
      return Module["asm"]["Wk"].apply(null, arguments);
    });
    var _mono_wasm_string_get_utf8 = (Module["_mono_wasm_string_get_utf8"] =
      function () {
        return Module["asm"]["Xk"].apply(null, arguments);
      });
    var _mono_wasm_string_from_js = (Module["_mono_wasm_string_from_js"] =
      function () {
        return Module["asm"]["Yk"].apply(null, arguments);
      });
    var _mono_wasm_get_obj_type = (Module["_mono_wasm_get_obj_type"] =
      function () {
        return Module["asm"]["Zk"].apply(null, arguments);
      });
    ...
    var _mono_wasm_array_length = (Module["_mono_wasm_array_length"] =
      function () {
    ...
    var _mono_wasm_obj_array_new = (Module["_mono_wasm_obj_array_new"] =
      function () {
        return Module["asm"]["cl"].apply(null, arguments);
      });
    var _mono_wasm_obj_array_set = (Module["_mono_wasm_obj_array_set"] =
      function () {
        return Module["asm"]["dl"].apply(null, arguments);
      });
    var _mono_wasm_string_array_new = (Module["_mono_wasm_string_array_new"] =
      function () {
    ...
    var _mono_wasm_enable_on_demand_gc = (Module[
      "_mono_wasm_enable_on_demand_gc"
    ] = function () {
      return Module["asm"]["kl"].apply(null, arguments);
    });
    var _mono_wasm_typed_array_new = (Module["_mono_wasm_typed_array_new"] =
      function () {
    ...
    var __emrtc_on_ice_candidate = (Module["__emrtc_on_ice_candidate"] =
      function () {
